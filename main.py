    ...
    for filename in filenames:
        with open(filename, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                score_dict[filename] = score_dict.get(filename, 0) + score
    return score_dict
...
if __name__ == '__main__':
    ...
    for dirpath, dirnames, filenames in os.walk('music'):
        for filename in filenames:
            filenames_.append(os.path.join(dirpath, filename))
            # print(os.path.join(dirpath, filename))

    score_dict = get_score(filenames_)
    with open('results.csv', 'w', encoding='utf-8') as f:
        for filename, score in score_dict.items():
            f.write(filename.split('\\')[1][:-4] + ',' + str(score) + '\n')
    print('done')
